...
app.get("/make-server-c192d0ee/health", (c) => {
  ...
  return c.json({ 
    ...
    endpoints: ["explain", "translate", "signup", "profile", "favorites", "text-to-speech"]
  });
});
  ...
  try {
    ...
    if (openaiApiKey) {
      try {
        ...
        if (isPersia) {
          const systemPrompt = `You are a Persian poetry interpreter trained in classical and modern Persian literature.
Your task is to produce deep, structured, and evidence-based interpretations (tafsir) suitable for display in a multilingual app interface.

Guidelines

Interpret the poem at three levels:
1. Literal meaning — what the text says directly.
2. Symbolic or mystical meaning — metaphors, Sufi, or philosophical layers.
3. Cultural or historical meaning — contextual significance, if relevant.

Identify literary devices with specific examples from the text (e.g. metaphor, ambiguity, allusion, antithesis, simile, pun, irony).

If the poet, form, or meter is unknown, return "unknown" and explain briefly.

Every claim or theme must cite at least one beyt index (couplet number).

If meaning is uncertain, include a short explanation under "uncertainty".

Always return valid JSON only — no extra text or commentary.
  ...
  "per_beyt": [
    {
      ...
      "literal": "string",
      "readings": [
        {"type": "symbolic|mystical|ethical|social|romantic", "text": "string", "confidence": 0.0}
      ],
      "notes": "short lexical or grammatical notes"
    }
  ],
  "glossary": [
...
TASK: Produce a detailed tafsir (interpretation) of Persian poetry in structured JSON.

INPUT:
- poem_original: <<<exact Persian text from Ganjoor>>>
- poet: <<<optional poet name>>>
- form|meter|rhyme: <<<optional from Ganjoor>>>
- needs: {depth: "standard|scholarly", show_meter: true, show_rhyme: true, max_symbols: 8}
...
- Provide at least 3 themes and 3 symbols when the poem allows.`;
          ...
          const userPrompt = `TASK: Produce a detailed tafsir (interpretation) of Persian poetry in structured JSON.

INPUT:
- poem_original: ${poem.text}
- poet: ${poem.poet?.name || 'unknown'}
- form|meter|rhyme: unknown
- needs: {depth: "standard", show_meter: true, show_rhyme: true, max_symbols: 8}
...
- Provide at least 3 themes and 3 symbols when the poem allows.`;
          ...
          try {
            timeoutId = setTimeout(() => {
              console.log(`Tafsir timeout after 20 seconds for poem ${poem.id}`);
              controller.abort();
            }, 20000); // 20 seconds timeout for comprehensive analysis
                ...
                try {
                  ...
                  const transformedExplanation = {
                    ...
                    lineByLine: tafsirData.per_beyt?.map(beyt => ({
                      ...
                      meaning: beyt.readings?.[0]?.text || 'معنا مشخص نشده'
                    })) || [],
